...
@Component("buyStrategy3")
public class BuyStrategyImpl3 implements BuyStockStrategy {
	@Autowired
	...
	public Map<String, Object> getBuyStrategyDecisions(String date, Double availableCash) {
		...
		for (StockTo stock : findStockByDate) {
			Double avgOfStock = stockService.countStocksAvgToDateAndDaysAmount(stock.getName(), date, 30);
		...
		Double ratio = price == 0.0 ? 0.0 : (availableCash / price) * 0.2;
